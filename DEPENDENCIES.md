...
| [`tokio`](https://github.com/tokio-rs/tokio) | 1 | MIT | One current-thread daemon/I/O shell, bounded channels, timers, task selection, child processes and pipe drains, and Unix termination signals. Direct production features are exactly `rt`, `sync`, `time`, `process`, `io-util`, `signal`, and `macros`; tests additionally enable `test-util` for paused time. PlasmaTop enables no Tokio filesystem, network, multithreaded runtime, or `full` feature. Normal direct transitives include `bytes`, `libc`, `mio`, `pin-project-lite`, `signal-hook-registry`, `socket2`, and the `tokio-macros` proc macro; zbus's Tokio integration also selects Tokio filesystem and networking support internally. |
