...
| `serde_json` | 1 | MIT OR Apache-2.0 | Structured tooltip and Graphs settings exchange between the widget and CLI, avoiding ambiguous delimiter parsing for ordered lists. Pure Rust; adds `itoa` and MIT-licensed `zmij`, reuses `memchr` and `serde_core`. `serde_json` and `zmij` include build scripts; neither links a native library. |
