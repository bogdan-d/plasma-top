...
| --- | --- | --- | --- |
| `nix` | 0.31 | MIT | Safe `flock(2)`, `getuid(2)`, and `poll(2)` wrappers. Pure Rust over `libc`; features limited to `fs`, `poll`, `process`, and `user`. Transitive: `bitflags`, `cfg-if`, `libc`; build helper: `cfg_aliases`. |
...
| [`signal-hook`](https://github.com/vorner/signal-hook) | 0.4 | MIT OR Apache-2.0 | Safe SIGINT/SIGTERM flag registration. Uses `signal-hook-registry` and `libc`; no native build step. |
