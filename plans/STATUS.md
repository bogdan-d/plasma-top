...
| CONFIG | 2 | active | Agent Manager `rust-p2-config` | `9a088c2` → `rust-p2-config` | in progress | Wave 2 fan-out 20260719; porting `config.py` to `rust/src/config/{mod,merge,geometry,assets}.rs`; ports `tests/test_config.py` (35 tests); proposes `toml`+`serde` deps; `_drop_unknown`/`_drop_misplaced` use frozen `ItemToken` API with local helpers pending DOMAIN integration |
| RUNTIME | 2 | active | Agent Manager `rust-p2-runtime` | `9a088c2` → `rust-p2-runtime` | in progress | Wave 2 fan-out 20260719; porting `runtime.py`/`pagestate.py` to `rust/src/runtime/{mod,atomic,page}.rs`; atomic writes + flock counter; lane chooses `nix` vs `libc`+scoped unsafe (proposes in handoff) |
| FIXTURES | 2 | active | Agent Manager `rust-p2-fixtures` | `9a088c2` → `rust-p2-fixtures` | in progress | Wave 2 fan-out 20260719; filling the `test_support` skeleton with concrete `FakeClock`/`FakeCommandRunner`/`FakeDbus`/`FixtureLoader`; mirrors BASE oracle fixture schema; proposes `toml` dep gated under `test-support` feature |
