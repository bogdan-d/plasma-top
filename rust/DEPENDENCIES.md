...
| [`wait-timeout`](https://github.com/alexcrichton/wait-timeout) | 0.2 | MIT OR Apache-2.0 | Safe timeout-bound waits for command adapters. `std::process` has no timed wait; this replaces polling/unsafe platform calls. No default features. Replacement is isolated to `ProductionCommandRunner`. | none; pure Rust over platform APIs | none | DAEMON-CLI lane |
| [`signal-hook`](https://github.com/vorner/signal-hook) | 0.3 | MIT OR Apache-2.0 | Safe SIGINT/SIGTERM flag registration for graceful daemon cleanup. Chosen over unsafe `sigaction` and handler-side I/O. Default features retained; no native dependency or packaging change. | none; pure Rust, no build script | `signal-hook-registry`, `libc` (already present) | DAEMON-CLI lane |
| [`serde_json`](https://github.com/serde-rs/json) | 1 | MIT OR Apache-2.0 | Decode `busctl --json=short` replies in the command-backed production D-Bus facade. Avoids native libdbus and keeps power logic behind the existing deterministic facade. Default features retained. Replacement is adapter-local. | none; pure Rust, no build script | `itoa`, `memchr`, `zmij`; reuses `serde_core` | DAEMON-CLI lane |
