...
AMDGPU uses one fast and one slow job on a serialized owner. Jobs read only demanded supported fields; configured graphs keep usage and codec history active while the tooltip is hidden. Enabling the temperature graph also demands the selected GPU's temperature, sampled from the AMD slow job with a 30-second freshness budget and copied into graph history on `display.history_interval`. Graphs do not demand power, fan, clock, or VRAM. Each field retains its own successful sample and capture time across failures. Confirmed capability loss clears only the affected field; confirmed device replacement clears all AMD samples and resets the selected GPU history. Discovery uses direct DRM/PCI/hwmon inspection and introduces no subprocess.
