...
`tools/p6_live_matrix.sh` uses a host `plasmoidviewer` when available and otherwise supports an existing Distrobox named `plasma-top-plasma-sdk`. Run each verification script with `--help` for its exact prerequisites and modes.
...
| Change | Verification |
|---|---|
| Render, CSS, or Qt RichText | `tools/p6_qt_matrix.sh --no-build` |
| Applet/runtime integration | `tools/qml_verify.sh --smoke` |
| Interactive application-form behavior | `tools/qml_verify.sh` |
| Panel orientation, geometry, hover, pinning, or wheel behavior | `tools/p6_live_matrix.sh` and, when needed, `tools/p6_live_matrix.sh --interactive` |
| Desktop representation or appearance settings | `tools/p6_live_matrix.sh --planar` |
...
The Qt matrix writes rendered HTML, PNGs, logs, an environment manifest, and a contact sheet under `.test-artifacts/p6/qt/`. Inspect `.test-artifacts/p6/qt/contact-sheet.png`; a passing rasterization is not a visual review. The live matrix writes geometry, command traces, and QML/daemon logs under `.test-artifacts/p6/live/`.
