...
struct RuntimeState {
    ...
    notification_samples: BTreeMap<RunId, DisplaySnapshot>,
    ...
    css_stamp: Option<SystemTime>,
    overlay_stamp: Option<SystemTime>,
    config_stamp: Option<SystemTime>,
    machine_stamps: Vec<Option<SystemTime>>,
    plasma_stamp: Option<SystemTime>,
    geom_stamp: Option<SystemTime>,
    kde_stamp: Option<SystemTime>,
    updates_stamp: Option<SystemTime>,
    server_stamp: Option<SystemTime>,
    panel_html: Option<String>,
    tooltip_html: Option<String>,
    display_publication_count: usize,
    first_paint_published: bool,
    theme_reconciliation_pending: bool,
    shutdown_requested: bool,
    action_queue: ActionQueue,
    resolved_mounts: Vec<String>,
    boot_pending: BTreeSet<&'static str>,
    terminate: bool,
}

impl RuntimeState {
    fn selected_page(&self, paths: &DaemonPaths) -> (usize, PageId) {
        let index = page_index(paths, self.active.len());
        (index, PageId::from_id(self.active[index].id))
    }

    fn scheduler_config(&self, _roots: &FilesystemRoots) -> crate::scheduler::SchedulerConfig {
        scheduler_config(
            &self.cfg,
            &self.hw,
