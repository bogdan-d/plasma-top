...
- Measure before optimizing. For this repo, inspect `docs/PERFORMANCE.md` before
  changing polling, watching, caching, RichText parsing, or publication cadence.
- Avoid unnecessary wrappers, clipping, layers, subtree opacity, shader effects,
  and frequently repainted `Canvas` content. Each can add traversal or offscreen
  rendering work.
- Keep delegates and frequently evaluated bindings small. Do not call shell
  commands, parse large text, or allocate large arrays from a hot binding.
